...
interface VastuService {
  title: string;
  description: string;
  icon: JSX.Element;
}

interface VastuCardSliderProps {
  services: VastuService[];
}

const VastuCardSlider = ({ services }: VastuCardSliderProps) => {
              ...
              >
                <div
                  className={`rounded-xl shadow-lg w-[250px] h-[350px] flex items-center justify-center overflow-hidden
                            ${isActive ? 'ring-2 ring-mystic-gold' : ''}`}
                  style={{
                    background: isActive
                    ...
                    {isActive && (
                      <div className="mt-4 transition-all duration-500 opacity-100 transform translate-y-0">
                        ...
                        <Link
                          ...
                          className="mt-2 inline-block bg-white text-mystic-gold px-5 py-2 rounded-md font-medium
          ...
          {services.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                index === activeIndex ? 'bg-mystic-gold w-6' : 'bg-gray-300'
              }`}
...
export default VastuCardSlider;